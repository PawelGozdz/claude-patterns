...
cmd_init() {
  ...
  # Detect project name from project.yml or current directory
  local project_name
  if [ -f ".claude/config/project.yml" ]; then
    project_name=$(grep -m1 '^  name:' .claude/config/project.yml | sed 's/^  name: *//;s/^"//;s/"$//')
  fi
