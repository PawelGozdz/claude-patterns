...
for (const p of rt.analyze?.panel ?? []) if (p.agent && !p.agent.startsWith('/')) agentNames.add(p.agent);
...
if (existsSync(settingsPath)) {
  ...
  const active = [...new Set([...raw.matchAll(/check-[a-z-]+/g)].map((m) => m[0]))];
...
console.log(bad.length ? '\n  ✗ setup niekompletny' : '\n  ✓ setup kompletny — /analyze i /orchestrate-blocks mają wszystko, na co wskazuje runtime.yml');
