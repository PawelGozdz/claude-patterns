...
- 2026-07-09 — architecture delegation target renamed `@technical-architecture-lead` → `@backend-technology-expert`
- 2026-07-03 — added `transactional-outbox-pattern.md` to the required Architecture reading list (outbox/rollback-leak rules)
- 2026-07-02 — added `StructuredOutput` to the tools whitelist (root-cause fix for the wf_8f8aeeb3 crash)
- 2026-07-02 — added turn-budget silent-death guard: emit a partial verdict with `unverified_scope:` at ~80% of `maxTurns` instead of dying silently
- 2026-07-02 — removed `mcp__zen__codereview`/`mcp__zen__analyze` from tools, `maxTurns` 15→30, added "verify the implementation exists first" pre-check and "read files whole, never partial" rule
- 2026-06-27 — Rule Cards system: verification gates rewritten to per-rule-ID checklists against `*_summary.md` cards, VETO triggers tied to specific rule IDs, removed `isolation: worktree`
- 2026-05-24 — editorial pass: removed verbose cost-justification prose, explicit Tools list, and Success Metrics section; condensed Verification Workflow/Collaboration to one-liners (v1.0.0 → v1.1.0)
- 2026-04-27 — added automatic VETO for missing per-file pattern citation, plus explicit triggers for `error.message` leakage and manual mock factories
- 2026-04-27 — added the full per-layer Pattern Knowledge Base section (domain/application/infrastructure/architecture/cross-layer/testing reading lists)
- 2026-03-30 — moved from `agents/verifiers/` to `agents/stacks/nestjs-ddd/`; fixed a stale `BUSINESS_RULES.md` reference to `.yaml`
