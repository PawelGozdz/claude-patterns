...
> **▶ STATUS: ZAMKNIĘTY 2026-09-07 — pilot poniżej progu, cały system wycofany (K99).**
>
> Kryterium go/no-go nie zostało spełnione, bo nie było czego oceniać: kanał zebrał
> **zero wpisów**, a kill-switch `/opt/projects/.claude-swarm/STOP` padł 2026-08-09
> (powód: koszt sesji stand-by), jedenaście dni przed terminem oceny. Przez kolejny
> miesiąc hooki dalej odpalały się w pięciu satelitach na każdy prompt i każdą edycję,
> dla kanału bez ani jednej wiadomości.
>
> Wykonane przy zamknięciu:
> - `cli.js install-hooks --remove` w 6 instancjach (`claude-patterns`, `juz-ide-api-1..4`,
>   `juz-ide-mobile-app`); kopie `settings.local.json.bak-2026-09-07` obok każdego pliku
> - usunięte z repo: `hooks/lib/broadcast/` (9 modułów), `hooks/broadcast-*.js` (3),
>   `commands/broadcast{,-status}.md`, `skills/orchestration/broadcast-standby/`,
>   `tests/flow-evals/broadcast/` (47 testów), `templates/broadcast/`, `docs/BROADCAST.md`,
>   wpisy w `hooks/hooks.json` i sekcja `[7b/8]` w `scripts/setup-project.sh`
> - kod w historii gita: **`4d4eac5181cdd45514a26d6d61e3956786d7c08a`**
>
> Do ręcznego sprzątnięcia przez operatora (pliki nieśledzone przez gita):
> `.claude/config/broadcast.yml` w 6 instancjach oraz `/opt/projects/.claude-swarm/`.
>
> Pełne uzasadnienie: [`docs/adr/0006-cross-instance-broadcast.md`](../adr/0006-cross-instance-broadcast.md),
> sekcja „Wynik pilotu i powód retire". Wszystko poniżej to zapis historyczny
> stanu z sierpnia 2026 — zostaje, bo D0-D11 i odrzucone alternatywy są nadal
> wartościowe dla kogoś, kto zaproponuje kanał peer-to-peer po raz drugi.
>
> ~~Instrukcja użycia dla człowieka: `docs/BROADCAST.md`~~ (usunięta razem z kodem).
