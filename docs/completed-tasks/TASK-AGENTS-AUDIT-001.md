...
**Status: DONE (2026-09-07)** — **wchłonięty przez `TASK-KAIZEN-002` (K54-K56, K86, K103)**.
**Źródło:** sesja 2026-07-07 (przy okazji budowy panelu code-review —
`reviewer-*`, `review-panel`, `pr-ops`, `api-contract-sync`)

> **Zamknięcie (2026-09-07).** Ten task był punktem wejścia, nie specyfikacją — i przez dwa
> miesiące nikt go nie podniósł, bo inwentaryzacja „wszystkiego naraz" nie ma naturalnego
> pierwszego kroku. Zrobił go audyt `docs/audits/2026-09-07-repo-audit.md`, który przeszedł
> całe repo i rozbił wynik na konkretne, wykonalne pozycje w `docs/tasks/TASK-KAIZEN-002.md`.
> Dalsza praca idzie tam, nie tutaj.
>
> Mapowanie Fazy 1 na pozycje KAIZEN-002:
>
> | punkt Fazy 1 | gdzie żyje teraz | stan |
> |---|---|---|
> | liczniki w README vs realna zawartość katalogów | **K70** (`count-assets.mjs --check-docs` w pre-commit) | otwarte |
> | martwe odnośniki wewnętrzne | **K103** (walidatory CI z prawdziwymi sprawdzeniami) + K11 z KAIZEN-001, który znalazł i naprawił 16 martwych referencji | w toku |
> | narzędzia MCP w `tools:` agentów (`mcp__zen__*`) | **K56** | ✅ 2026-09-07 — 16 plików, grep daje 0 |
> | `presets/*.yml` pod kątem przemianowanych agentów | bezprzedmiotowe — presety usunięte przez [ADR 0008](../adr/0008-stack-blocks-composition.md) 2026-08-12; odpowiednik to spójność slotów w `blocks/**` | — |
>
> Faza 2 („decyzje") rozpłynęła się w konkretach: **K54** (`tech-lead` na Sonnecie),
> **K55** (placeholdery zamiast zmyślonych liczb w `business-rules-auditor`),
> **K86** (karty reguł nieosiągalne przez żaden mechanizm wymuszony) i **K103**
> (automatyzacja, o którą prosił punkt „rozważyć automatyzację").
