...
| **GEO8** | One canonical predicate builder; express the column kind in the **type** (discriminated union), not a comment. **Shipped**: `src/shared/geo/` — `buildSpatialPredicate`/`buildDistanceExpression`/`buildKnnOrderExpression` over `SpatialSource`. Use it; don't hand-roll a call site. | Five parallel implementations, each with its own answer on casting — the observed steady state |
