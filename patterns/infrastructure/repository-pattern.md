...
**Rule card**: [`repository-pattern_summary.md`](./repository-pattern_summary.md) (RP1-RP12, N1-N7) — that
card is what gets injected into implementer prompts; this file is the reasoning behind it.

## When to Use

**Use this pattern for:**
- ✅ any class that reads or writes aggregate state through Kysely — command side (`*-command-kysely.repository.ts`) and query side (`*-query-kysely.repository.ts`) alike
- ✅ deciding which of the two shapes a new repository takes (write-side inherits `BaseKyselyRepository`, read-side does not — see the decision tree below)
- ✅ wiring optimistic locking, upsert configuration and `TransactionHost` into a write-side repository
- ✅ cron jobs, schedulers and application services that need aggregate data — they go through the repository port, never through a raw DB client (RP12/N7)

**Do NOT use for:**
- ❌ the `eventMap` reconstruction rules specifically — that is `repository-events-pattern.md`, which this pattern references rather than repeats
- ❌ domain ↔ persistence conversion detail — that is `mapper-pattern.md`; here the mapper is only injected and called
- ❌ PostGIS/spatial predicates and index-shape matching — that is `geo-spatial-query-pattern.md`
- ❌ outbox delivery and cross-context publication — that is `transactional-outbox-pattern.md` and `integration-event-pattern.md`
...
11. **MUST reach aggregate data through a repository port** - cron jobs, schedulers and application
    services inject the interface from `domain/repositories/`, never a raw `Kysely<Database>` /
    `DATABASE_TOKEN` (RP12). A method the port is missing is a signal to add it to the port.
...
8. **NEVER inject a raw DB client outside `infrastructure/repositories/`** - not without a
   `// RP12-EXCEPTION: <reason>` comment and an entry in the Exceptions section (N7)
...
### 4. Wrong Aggregate Version Table (Cross-Context Write Contention)
...
### 5. Manual Transaction Management (Connection Pool Exhaustion)
...
### 6. Throwing Exceptions (Should Return Result)
...
### 7. Missing Event Registration (Runtime Error)
...
### 8. Raw DB Client in a Cron / Scheduler / Application Service (Layer Bypass)

The first live `/orchestrate-ddd` run in `juz-ide-api-1` (TS-SEC-ANTI-SPOOF-003, 2026-07-05)
produced this. The port had no `purgeExpired()` yet, so the implementer took the shortcut instead of
adding the method. Human review caught it; a survey of the other 18 crons and schedulers in that
repo found none doing the same, so it was the single outlier — which is exactly the shape a rule
should catch, because one exception is what a convention erodes from.

```typescript
// ❌ WRONG: cron talks to the database directly
@Injectable()
export class DeviceNoncePurgeCron {
  constructor(@Inject(DATABASE_TOKEN) private readonly db: Kysely<Database>) {}

  @Cron('0 * * * *')
  async purge(): Promise<void> {
    await this.db.deleteFrom('device_nonces').where('expires_at', '<', new Date()).execute();
  }
}
```

Two costs, both paid later: the cron cannot be unit-tested without a real database client, and the
delete bypasses whatever the repository does around that table (soft-delete, audit, event dispatch).

```typescript
// ✅ CORRECT: the port grows a method, the cron stays thin
// domain/repositories/device-nonce.repository.ts
export interface IDeviceNonceRepository {
...
export class DeviceNoncePurgeCron {
  constructor(
    @Inject(DEVICE_NONCE_REPOSITORY) private readonly repo: IDeviceNonceRepository,
  ) {}

  @Cron('0 * * * *')
  async purge(): Promise<void> {
    const removed = await this.repo.purgeExpired(new Date());
    this.logger.log({ removed }, 'device nonces purged');
  }
}
```

**Exceptions** are real and allowed — a cross-aggregate bulk operation where a per-aggregate
repository would be an artificial detour, or a poller working outside a single aggregate boundary
(`KyselyOutboxRepository` is the in-tree example). They require a `// RP12-EXCEPTION: <reason>`
comment above the injection and an entry in the Exceptions section of this file. A raw token with
neither is a VETO, not "they probably had a reason" — same standard as the RP2 exception.

---

## Exceptions

Deviations from RP2 (command repository without `extends BaseKyselyRepository`) and RP12 (raw DB
client outside `infrastructure/repositories/`) are recorded here, one entry per case: the file, the
reason, and what was checked before accepting it. An empty section means no accepted deviations —
...
- **geo-spatial-query-pattern.md** - PostGIS predicates, cast shape = index shape, spatial predicate as access control

---

## Choosing Command vs Query Repository
...
**Created**: 2026-01-04 · **Restored from git history**: 2026-09-07 (the file had been slimmed to a
20-line stub in `64906b8` while six `pattern_routing:` rules and `patterns/README.md` still pointed here)
