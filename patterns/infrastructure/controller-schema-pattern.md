...
- ✅ every HTTP controller in `infrastructure/controllers/` — the shape of the class, its decorators, and what it is allowed to do
- ✅ getting the acting user right: `@CurrentUser` from the JWT, never a `userId` field in the body (ADR-0021)
- ✅ returning `Result<z.infer<typeof schema>>` instead of a hand-rolled DTO, so the response type follows the schema
- ✅ keeping orchestration-only controllers: parse, delegate to the command/query bus, map the `Result`

**Do NOT use for:**
- ❌ writing the schemas themselves — `zod-schema-validation-pattern.md`
- ❌ throttling and abuse limits — `rate-limit-guard-pattern.md`
- ❌ anything the handler should do (loading aggregates, applying rules, deciding outcomes) — `command-handler-pattern.md`, `query-handler-pattern.md`
- ❌ mapping a domain error to an HTTP status — that lives in the error mapper (`domain-errors-pattern.md`)
...
The Zod request/response schemas this controller imports — `.strict()`, UUID ids, safe-text
patterns, shared `commonValidators`, and the `AuthorSnapshotDto` fragment — are in
[`zod-schema-validation-pattern.md`](./zod-schema-validation-pattern.md).

---
### Rate limits

The `@RateLimit` decorator and the per-context `*.rate-limits.ts` file it reads from are in
[`rate-limit-guard-pattern.md`](./rate-limit-guard-pattern.md).
...
### 2. Business Logic in Controller (Should be in Handler)
...
### 3. Returning Raw DTOs (No Type Safety)
...
### 4. Using Generic `new Error()` (Wrong Error Type)
...
### 5. Adding `success` Field (Duplicates JSend)
