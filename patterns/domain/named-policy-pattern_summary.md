...
| **NP5** | An intentional exemption (e.g. "organizations skip the residence guardrail") is a visible, named line INSIDE the policy factory — never expressed only by a call's absence on one branch. | A reviewer or a guardian check (CH11, `command-handler-pattern_summary.md`) cannot distinguish "deliberate exemption" from "someone forgot the call" when the only evidence is what's missing. |
